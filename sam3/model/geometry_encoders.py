...
def concat_padded_sequences_old(seq1, mask1, seq2, mask2, return_index: bool = False):
    """
...
def concat_padded_sequences(seq1, mask1, seq2, mask2, return_index=False):
    seq1_length, B, C = seq1.shape
    seq2_length = seq2.shape[0]

    # Compute actual lengths
    actual_seq1_lengths = (~mask1).sum(dim=-1)  # [B]
    actual_seq2_lengths = (~mask2).sum(dim=-1)  # [B]
    ...
    # Build new mask
    concatenated_mask = (
        torch.arange(max_length, device=seq1.device)[None] >= final_lengths[:, None]
    )

    # Init result buffer
    out = seq1.new_zeros(max_length, B, C)

    # Copy seq1 (already correctly padded)
    out[:seq1_length] = seq1

    # Construct per-batch index mapping for seq2
    if return_index:
        all_indices = []

    for b in range(B):
        start = actual_seq1_lengths[b].item()
        end   = start + actual_seq2_lengths[b].item()

        # copy only the valid (unpadded) tokens of seq2 for this batch
        valid_len = actual_seq2_lengths[b].item()
        out[start:end, b] = seq2[:valid_len, b]

        if return_index:
            idx = torch.arange(start, end, device=seq1.device)
            # pad to seq2_length for shape consistency
            pad = seq2_length - valid_len
            if pad > 0:
                idx = torch.cat([idx, idx.new_full((pad,), -1)])
            all_indices.append(idx)

    if return_index:
        index = torch.stack(all_indices, dim=1)  # [seq2_length, B]
        return out, concatenated_mask, index

    return out, concatenated_mask
